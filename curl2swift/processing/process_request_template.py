...
def process_request_template(
    ...
    for row, header in zip(content.header_rows, content.headers):
        ...
        if header not in dynamic_values[ParameterType.HEADER]:
            header_rows.append("setHeader(." + enum_case + ', "' + content.headers[header] + '")')
    ...
    for row, param_pair in zip(content.body_param_rows, content.param_names):
        ...
        if param_pair[0] not in dynamic_values[ParameterType.BODY_PARAM]:
            body_param_rows.append("setBodyParameter(." + enum_case + ', "' + param_pair[1] + '")')
    ...
    if dynamic_values[ParameterType.PATH_PARAM] or dynamic_values[ParameterType.QUERY_PARAM] or dynamic_values[ParameterType.HEADER] or dynamic_values[ParameterType.BODY_PARAM]:
